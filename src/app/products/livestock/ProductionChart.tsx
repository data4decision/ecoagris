...
export default function ProductionChart() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        setError('Error loading production data');
        setLoading(false);
      }
    }
    fetchData();
  ...
  const options = {
    ...
    plugins: {
      ...
      title: { display: true, text: `Production Trends in ${country.charAt(0).toUpperCase() + country.slice(1)}`, color: 'var(--dark-green)' },
    },
    scales: {
      x: { title: { display: true, text: 'Year', color: 'var(--olive-green)' } },
      y: { title: { display: true, text: 'Tons', color: 'var(--olive-green)' } },
    },
  };
  ...
  return (
    <div className="bg-[var(--white)] p-4 rounded-lg shadow">
      <h2 className="text-lg font-semibold text-[var(--dark-green)] mb-4">
        Production and Output in {country.charAt(0).toUpperCase() + country.slice(1)}
      </h2>
