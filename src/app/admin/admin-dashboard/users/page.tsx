...
interface Admin {
  ...
  name: string;
  role: string;
  createdAt: unknown;  // Use 'unknown' to ensure Vercel compatibility
}
...
export default function ManageAdmins() {
  ...
  const handleAddAdmin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Save user profile — MUST include uid: uid in the 'newAdmin' collection
      ...
      setEmail('');
      setPassword('');
      setAdmins((prev) => [...prev, { uid, email, name, role: 'admin', createdAt: serverTimestamp() }]);
    } catch (err: any) {
      console.error('Create error:', err);
      let text = 'Failed to add admin.';
      if (err.code === 'auth/email-already-in-use') text = 'Email already registered.';
      else if (err.code === 'auth/weak-password') text = 'Password too weak.';
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-xl shadow-lg p-6">
            ...
            <form onSubmit={handleAddAdmin} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-[var(--dark-green)] mb-1">
                  <FaEnvelope className="inline mr-1" /> Email (@ecoagris.org)
                </label>
                <input
                  type="email"
                  value={email}
                  ...
                  >
                    <div>
                      <p className="font-medium text-[var(--dark-green)] flex items-center gap-2">
                        <FaUser className="text-sm" /> {a.name}
                      </p>
